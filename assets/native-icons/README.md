...
`masters/` contains 42 SVG masters: 40 active canonical icons and two retained historical song-notification masters (`notifications.svg` and `notifications-off.svg`). The active names are:
...
`update` and `update-available` were designed for Nativune in September 2026 as original glyphs under the repository's MIT License.
