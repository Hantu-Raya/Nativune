...
internal static class Program
{
    ...
    {
        Console.WriteLine("OAuthProbe - OAuth/library and Windows media capability probe");
        Console.WriteLine();
        Console.WriteLine("Usage: OAuthProbe [--root <project-root>] <command>");
        Console.WriteLine();
...
internal static class Dpapi
{
    ...
    public static byte[] Protect(byte[] plain)
    {
        ...
        try
        {
            if (!CryptProtectData(ref input, "OAuthProbe token", IntPtr.Zero, IntPtr.Zero, IntPtr.Zero, CryptProtectUiForbidden, out output))
...
internal static class SelfCheck
{
    public static int Run(string root)
    {
        ...
        try
        {
            ...
            Console.WriteLine("Self-check passed: DPAPI, loopback state, credential endpoints, WebView navigation origin boundary, and empty/malformed YouTube lists.");
            return 0;
        }
        finally
        {
