...
internal static class CompactViewChecks
{
    ...
    internal static async Task RunNativeAsync(WebHostWindow host)
    {
        ...
        Action<string, double?> recordCommand = (command, value) => commands.Add((command, value));
        view.CommandRequested += recordCommand;
        ...
        commands.Clear();
        // Stand-in for a page that has not answered yet: only this check hears the view's commands,
        // so the page-less fixture host cannot answer "not sent" and re-arm the gate early.
        var hostCommandHandlers = ReadField(view, "CommandRequested");
        SetField(view, "CommandRequested", recordCommand);
        ...
        view.PlaylistsRequested -= CountRequest;
        view.PlaylistChosen -= Choose;
        SetField(view, "CommandRequested", hostCommandHandlers);
        commands.Clear();
        ...
        // As in the rating phase, only this check hears the view's commands during seek gestures.
        SetField(view, "CommandRequested", recordCommand);
        var seek = parts["Seek"];
